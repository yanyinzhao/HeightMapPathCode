...
#include "simple_global_function.h"
...
namespace geodesic
{
	...
	public:
		...
		void add_vertex(double added_x, double added_y, double added_z);

		std::vector<Vertex> &vertices() { return m_vertices; };
		std::vector<Edge> &edges() { return m_edges; };
		std::vector<Face> &faces() { return m_faces; };
		std::unordered_map<unsigned, int> &vert_coor_and_id_map() { return m_vert_coor_and_id_map; };
		...
		int m_x_vertex_num, m_y_vertex_num;
		unsigned m_hash_max;
	...
	private:
		...
		std::unordered_map<unsigned, int> m_vert_coor_and_id_map;
									...
									Faces &tri)
	{
		...
		for (unsigned i = 0; i < num_vertices; ++i) // copy coordinates to vertices
		{
			...
			v.y() = p[shift + 1];
			v.z() = p[shift + 2];
			assert(std::floor(v.x()) == std::ceil(v.x()));
			assert(std::floor(v.y()) == std::ceil(v.y()));
			assert(std::floor(v.z()) == std::ceil(v.z()));
		}
	...
	inline void Mesh::build_adjacencies()
	{
		...
		for (unsigned i = 0; i < m_edges.size(); ++i)
		{
			...
			// if ((e.adjacent_vertices()[0]->id() == 378 && e.adjacent_vertices()[1]->id() == 403) ||
			// 	(e.adjacent_vertices()[0]->id() == 403 && e.adjacent_vertices()[1]->id() == 378))
			// {
		...
		for (unsigned i = 0; i < m_edges.size(); ++i)
		{
			...
			if (e.is_boundary())
			{
				...
				e.adjacent_vertices()[0]->boundary() = true;
				e.adjacent_vertices()[1]->boundary() = true;
			}
		}
	...
	inline bool Mesh::verify() // verifies connectivity of the mesh and prints some debug info
	{
		...
		m_y_vertex_num = y_vertex_num;

		m_hash_max = unsigned(std::max(m_width, m_height));

		for (unsigned i = 0; i < m_vertices.size(); ++i)
		{
			Vertex &v = m_vertices[i];
			unsigned x_y;
			assert(std::floor(v.getx() - m_xmin) == std::ceil(v.getx() - m_xmin));
			assert(std::floor(v.gety() - m_ymin) == std::ceil(v.gety() - m_ymin));
			hash_function_two_keys_to_one_key(m_hash_max, unsigned(v.getx() - m_xmin), unsigned(v.gety() - m_ymin), x_y);
			m_vert_coor_and_id_map[x_y] = v.id();
		}

		return true;
	}

	void Mesh::add_vertex(double added_x, double added_y, double added_z)
	{
		Vertex v;
